    ...
    <div className="animate-fade-in max-w-md mx-auto py-8">
      ...
      {tab === 'signin' && (
        <form onSubmit={handleEmailSignIn} className="space-y-4 animate-fade-in">
          ...
          <button type="submit" disabled={submitting} className="w-full py-3.5 bg-black text-white rounded-full font-medium text-xs uppercase tracking-widest hover:bg-black/80 disabled:opacity-50 transition-colors">{submitting ? 'Signing in...' : 'Sign In'}</button>
        </form>
      )}
      ...
      {tab === 'signup' && (
        <form onSubmit={handleEmailSignUp} className="space-y-4 animate-fade-in">
                  ...
                  <div key={cat}>
                    <button type="button" onClick={() => setExpandedCat(isExpanded ? null : cat)} className="w-full flex items-center justify-between px-3 py-2 text-sm hover:bg-neutral-50 rounded-lg">
                      <span className="flex items-center gap-2">
                        ...
                        {selectedCount > 0 && <span className="bg-black text-white text-xs px-1.5 py-0.5 rounded-full">{selectedCount}</span>}
                      </span>
                    ...
                    {isExpanded && (
                      <div className="pl-3 pb-2 grid grid-cols-1 gap-1">
                        {conditions.map((c) => (
                          <button key={c.id} type="button" onClick={() => toggleDietary(c.id)} className={clsx('text-left px-3 py-1.5 rounded-lg text-xs transition-colors', dietaryPrefs.includes(c.id) ? 'bg-black/10 text-black font-medium' : 'text-neutral-500 hover:bg-neutral-50')}>{c.label}</button>
                        ))}
                      </div>
                    )}
                  </div>
          ...
          <button type="submit" disabled={submitting} className="w-full py-3.5 bg-black text-white rounded-full font-medium text-xs uppercase tracking-widest hover:bg-black/80 disabled:opacity-50 transition-colors">{submitting ? 'Creating account...' : 'Create Account'}</button>
        </form>
      )}
    </div>
  );
