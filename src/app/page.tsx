  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-[#2d2d2a] font-[family-name:var(--font-serif)] mb-1">What&apos;s in your kitchen?</h1>
          <p className="text-[#7a7568] text-sm">Type your ingredients or snap a photo of what you have.</p>
        </div>
        <div>
          <label className="block text-sm font-semibold text-[#5a5347] mb-2">Add ingredients manually</label>
          <IngredientInput onAdd={addIngredient} />
        </div>
        <div>
          <label className="block text-sm font-semibold text-[#5a5347] mb-2">Or upload food photos</label>
          <PhotoUpload onExtract={handlePhotoExtract} isExtracting={isExtracting} />
        </div>
        {ingredients.length > 0 && (
          <div className="animate-fade-in">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-sm font-semibold text-[#5a5347]">Your ingredients ({ingredients.length})</h2>
              <button onClick={clearIngredients} className="text-xs text-[#a89f94] hover:text-red-500 transition-colors">Clear all</button>
            </div>
        ...
        <button onClick={handleNext} disabled={ingredients.length === 0}
          className="w-full py-3.5 bg-olive-600 text-white rounded-xl font-semibold text-sm hover:bg-olive-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
